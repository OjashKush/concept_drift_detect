...
def cusum(data, threshold):
    ...
    for i in range(1, n):
        cumulative_sum[i] = np.maximum(0, cumulative_sum[i-1] + data[i] - np.mean(data, axis=0) - threshold)
...
def main():
    ...
    # Apply cumulative sum using np.cumsum
    cumulative_sum_primary = np.cumsum(reduced_primary, axis=0)

    cp_1 = change_points.rpt_pelt(reduced_primary, pen=pen_primary)
    cp_1 = cp_1[:len(reduced_primary)]  # Ensure change points length matches the reduced series
    print("Change points primary:", cp_1)
    ...
    print("Change points secondary:", cp_2)
    ...
    change_points_primary = determine_change_points_primary(cp_1)
