            ...
            if cp_2 < cp_1:
                ...
                for i in range(0,len(secondary_features)):
                    ...
                    for f_2 in primary_features:
                        # Remove the column with constant values from the data frame.
                        constant_columns = []
                        for column in f_2:
                            if np.std(f_2[column]) == 0:
                                constant_columns.append(column)
                        if len(constant_columns) > 0:
                            f_2 = pd.DataFrame(f_2).drop(constant_columns, axis=0)
                        
                        data_frame = pd.DataFrame(f_2)
                        data_frame[secondary_feature_names[i]] = f
                        data_frame = data_frame.dropna()
                        try:
                            gc_res = grangercausalitytests(data_frame, [k], verbose = False)
